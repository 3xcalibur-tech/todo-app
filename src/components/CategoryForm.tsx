...
export const CategoryForm: React.FC<CategoryFormProps> = React.memo(
  ({ categoryId, onClose }) => {
    ...
    }, [categoryId]); // Only depend on categoryId, not the entire categories array
    ...
    return (
      <motion.div
        className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 form-overlay"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        onClick={onClose}
      >
        <motion.div
          className="bg-white dark:bg-gray-800 rounded-xl p-6 w-96 shadow-xl bg-transition"
